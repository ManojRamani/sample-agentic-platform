...
if [[ -f "$SRC_AGENT_DOCKERFILE_PATH" ]]; then
    ...
    echo "Available services in src/agentic_platform/service/:"
    ls -1 src/agentic_platform/service/ 2>/dev/null | grep -v "^$" || echo "  No services found in src/agentic_platform/service/ directory"
    exit 1
fi
