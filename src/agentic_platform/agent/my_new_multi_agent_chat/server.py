"""FastAPI server for the Multi-Agent Chat Agent."""
...
from agentic_platform.agent.my_new_multi_agent_chat.controller import multi_agent_chat_controller
...
app = FastAPI(title="Multi-Agent Chat Agent")

# Configure middleware
configuration_server_middleware(app, path_prefix="/api/multi-agent-chat")

@app.post("/invocations", response_model=AgenticResponse)
@app.post("/invoke", response_model=AgenticResponse)
# @handle_exceptions(status_code=500, error_prefix="Multi-Agent Chat Agent API Error")
async def invoke(request: AgenticRequest) -> AgenticResponse:
    """
    Invoke the Multi-Agent Chat agent with a standard response.
    """
    return await multi_agent_chat_controller.invoke(request)


@app.post("/stream")
@handle_exceptions(status_code=500, error_prefix="Multi-Agent Chat Agent Streaming API Error")
async def stream(request: AgenticRequest) -> StreamingResponse:
    """
    Stream responses from the Multi-Agent Chat agent using Server-Sent Events.
    """
    async def event_generator():
        """Convert StreamEvent objects to SSE format with event names."""
        try:
            async for stream_event in multi_agent_chat_controller.create_stream(request):
