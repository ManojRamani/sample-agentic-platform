#!/usr/bin/env python3
"""
EKS integration test runner for agentic_chat_enhanced agent with authentication.
...
class EKSAgenticChatEnhancedTesterWithAuth:
    """EKS integration tester for agentic_chat_enhanced agent with authentication."""
    ...
    def test_agentic_chat_enhanced_service_exists(self) -> bool:
        """Test that agentic-chat-enhanced service exists."""
        print("\n🔍 Testing agentic-chat-enhanced service exists...")

        success, output = self.run_kubectl_command(
            ["kubectl", "get", "service", "agentic-chat-enhanced", "-o", "json"]
        )
    ...
    def test_agentic_chat_enhanced_pods_running(self) -> bool:
        """Test that agentic-chat-enhanced pods are running."""
        print("\n🔍 Testing agentic-chat-enhanced pods are running...")

        success, output = self.run_kubectl_command(
            ["kubectl", "get", "pods", "-l", "app=agentic-chat-enhanced", "-o", "json"]
        )
        ...
        if success:
            try:
                ...
                if len(pods) == 0:
                    print("❌ No agentic-chat-enhanced pods found")
                    return False
    ...
    def start_port_forward(self) -> bool:
        """Start port forwarding for agentic-chat-enhanced service."""
    ...
    async def test_invoke_endpoint_with_auth(self) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                # Test enhanced agent with calculator
                payload = {
                    "message": {
                        "role": "user",
                ...
                if response.status_code == 200:
                    ...
                    # Check for intelligent response with calculator result
                    if any(
        ...
        async with httpx.AsyncClient() as client:
            try:
                payload = {
                    "message": {
                        "role": "user",
                        "content": [{"type": "text", "text": "Calculate 12 * 8 step by step"}],
                    },
                    "session_id": "eks-streaming-test-enhanced-auth",
                }
    ...
    def test_service_logs(self) -> bool:
        ...
        success, output = self.run_kubectl_command(
            ["kubectl", "logs", "-l", "app=agentic-chat-enhanced", "--tail=20", "--since=10m"]
        )
    ...
    async def run_all_tests(self) -> bool:
        ...
        print("🚀 EKS Integration Tests for Agentic Chat Enhanced Agent (With Auth)")
        print("=" * 80)
        ...
        tests = [
            ...
            ("Agentic Chat Enhanced Service Exists", self.test_agentic_chat_enhanced_service_exists),
            ("Agentic Chat Enhanced Pods Running", self.test_agentic_chat_enhanced_pods_running),
...
async def main():
    """Main test runner."""
    tester = EKSAgenticChatEnhancedTesterWithAuth()
