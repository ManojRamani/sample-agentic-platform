#!/usr/bin/env python3
"""
EKS integration test runner for my_new_multi_agent_chat agent with authentication.
...
class EKSMyNewMultiAgentChatTesterWithAuth:
    """EKS integration tester for my_new_multi_agent_chat agent with authentication."""
    ...
    def test_my_new_multi_agent_chat_service_exists(self) -> bool:
        """Test that my-new-multi-agent-chat service exists."""
        print("\n🔍 Testing my-new-multi-agent-chat service exists...")

        success, output = self.run_kubectl_command(
            ["kubectl", "get", "service", "my-new-multi-agent-chat", "-o", "json"]
        )
    ...
    def test_my_new_multi_agent_chat_pods_running(self) -> bool:
        """Test that my-new-multi-agent-chat pods are running."""
        print("\n🔍 Testing my-new-multi-agent-chat pods are running...")

        success, output = self.run_kubectl_command(
            ["kubectl", "get", "pods", "-l", "app=my-new-multi-agent-chat", "-o", "json"]
        )
        ...
        if success:
            try:
                ...
                if len(pods) == 0:
                    print("❌ No my-new-multi-agent-chat pods found")
                    return False
    ...
    def start_port_forward(self) -> bool:
        """Start port forwarding for my-new-multi-agent-chat service."""
        print("\n🔍 Starting port forwarding...")

        try:
            # Start port forwarding in background
            self.port_forward_process = subprocess.Popen(
                ["kubectl", "port-forward", "service/my-new-multi-agent-chat", "8004:80"],
                env=os.environ.copy(),
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(
                    "http://localhost:8004/health", timeout=10.0
                )
    ...
    async def test_invoke_endpoint_with_auth(self) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                # Test multi-agent coordination with a calculation task
                payload = {
                    "message": {
                        "role": "user",
                        "content": [{"type": "text", "text": "Calculate 15 * 8 and explain the process"}],
                    },
                    "session_id": "eks-integration-test-multi-agent-auth",
                }
                ...
                if response.status_code == 200:
                    ...
                    # Check for multi-agent response indicators
                    if any(
                        ...
                        print("✅ Response contains expected multi-agent calculation result")
                        return True
                    else:
                        print(
                            "⚠️  Response doesn't contain expected multi-agent keywords"
                        )
        ...
        async with httpx.AsyncClient() as client:
            try:
                payload = {
                    "message": {
                        "role": "user",
                        "content": [{"type": "text", "text": "Create a short creative story about robots"}],
                    },
                    "session_id": "eks-streaming-test-multi-agent-auth",
                }
    ...
    def test_service_logs(self) -> bool:
        ...
        success, output = self.run_kubectl_command(
            ["kubectl", "logs", "-l", "app=my-new-multi-agent-chat", "--tail=20", "--since=10m"]
        )
    ...
    async def run_all_tests(self) -> bool:
        ...
        print("🚀 EKS Integration Tests for My New Multi Agent Chat Agent (With Auth)")
        print("=" * 70)
        ...
        tests = [
            ...
            ("My New Multi Agent Chat Service Exists", self.test_my_new_multi_agent_chat_service_exists),
            ("My New Multi Agent Chat Pods Running", self.test_my_new_multi_agent_chat_pods_running),
...
async def main():
    """Main test runner."""
    tester = EKSMyNewMultiAgentChatTesterWithAuth()
